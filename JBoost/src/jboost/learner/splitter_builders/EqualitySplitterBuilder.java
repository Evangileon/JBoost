package jboost.learner.splitter_builders;
...
public class EqualitySplitterBuilder extends SplitterBuilder {
  ...
  public CandidateSplit build() {
    Bag[] attributeValueBags = new Bag[attr_val.length]; 
    Bag universe = booster.newBag();
    int j;
    for (j = 0; j < attr_val.length; j++) {
      attributeValueBags[j] = intersect(examplesMask, attr_val[j]);
      universe.addBag(attributeValueBags[j]);
    }
    ...
    for (j = 0; j < attributeValueBags.length; j++) {
      tmpBags[1].copyBag(universe);
      tmpBags[0] = attributeValueBags[j];
    ...
    tmpBags[1].copyBag(universe);
    tmpBags[0] = attributeValueBags[minVal];
    tmpBags[1].subtractBag(tmpBags[0]);
...
public void finalizeData() {
  ...
  /**
   * This method returns a bag which contains elements that are the result of intersection between
   * mask and examples. In other words, if mask[examples[i]] is true, this example is in a bag result.
   * <br>
   * For example, given that<br>
   * mask = { true,true,true,true,true,false }<br>
   * examples = { 0,2,5 } <br>
   * The result is a bag containing elements no. 0 and 2.
   * 

...
public class TextDescription extends AttributeDescription {
  ...
  /**
	 * 
	 */
	private static final long serialVersionUID = 8448340357693678482L;

TextDescription(String name, Configuration c) throws ClassNotFoundException {
    attributeName = name;
    attributeClass = Class.forName("jboost.examples.attributes.SetAttribute");
  ...
  public Attribute str2Att(String string) throws BadAttException {
    ...
    HashMap<String,LocWord> locMap = new HashMap<String,LocWord>();
    ...
    try {
      ...
      for (int i = 0; i < wordArr.length; i++) {
        ...
        if (locMap.containsKey(word)) { // word appeared in curr text
          ...
          if (!wt.frozen && !wt.map.containsKey(word)) { // string is nowhere
            Monitor.log("adding to wordTable word:" + word + ", token=" + wt.size,Monitor.LOG_LEVEL_THIRTY);
      ...
      Monitor.log(e.getMessage() + e,Monitor.LOG_LEVEL_THREE);

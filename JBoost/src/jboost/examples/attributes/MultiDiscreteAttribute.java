...
public class MultiDiscreteAttribute extends Attribute {
  ...
  public static void main(String[] args) {
    ...
    for (i = 0; i < a.length; i++) {
      Monitor.log("i = " + i,Monitor.LOG_LEVEL_THREE);
      if(Monitor.getLogLevel() > Monitor.LOG_LEVEL_THREE)
    	  Monitor.log("getSingleValue = " + a[i].getSingleValue(),Monitor.LOG_LEVEL_THREE);
      for (j = 0; j < 6; j++) {
    	  if(Monitor.getLogLevel() > Monitor.LOG_LEVEL_THREE)
    		  Monitor.log("getMultiValue(" + j + ") = " + a[i].getMultiValue(j),Monitor.LOG_LEVEL_THREE);
      }
      for (j = 0; j < a.length; j++)
    	  if(Monitor.getLogLevel() > Monitor.LOG_LEVEL_THREE)
    		  Monitor.log(i + (a[i].equals(a[j]) ? "=" : "!=") + j,Monitor.LOG_LEVEL_THREE);
    }
  }
}

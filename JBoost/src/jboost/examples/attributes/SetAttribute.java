...
import jboost.examples.attributes.descriptions.TextDescription;
import jboost.exceptions.RepeatedElementException;
...
public class SetAttribute extends Attribute {
  ...
  public static void main(String[] argv) {
    try {
      Monitor.log("starting to test SetAttribute",Monitor.LOG_LEVEL_THREE);
      int[] s = { 5, 2, 3, 7 };
      SetAttribute a = new SetAttribute(s);
      if(Monitor.getLogLevel() > Monitor.LOG_LEVEL_THREE) {
    	  Monitor.log(a,Monitor.LOG_LEVEL_THREE);
      	  Monitor.log("The element 2 is in the set:  " + a.contains(2),Monitor.LOG_LEVEL_THREE);
      	  Monitor.log("The element 4 is in the set:  " + a.contains(4),Monitor.LOG_LEVEL_THREE);
      }

      Monitor.log("testing exception throwing",Monitor.LOG_LEVEL_THREE);
      s[0] = 7;
      try {
      ...
      Monitor.log("finished testing SetAttribute",Monitor.LOG_LEVEL_THREE);
    }
    catch (Exception e) {
      Monitor.log(e.getMessage(),Monitor.LOG_LEVEL_THREE);
      e.printStackTrace();
